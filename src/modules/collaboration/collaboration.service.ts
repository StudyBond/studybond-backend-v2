...
export class CollaborationService {
  ...
  async publishParticipantFinishedRealtime(input: {
    sessionId: number;
    userId: number;
    examId: number;
    finishedAt: string;
    result: CollaborationSubmissionFinalizationResult;
  }): Promise<void> {
    if (input.result.participantFinishedNow) {
      await this.publishSessionEvent(input.sessionId, {
