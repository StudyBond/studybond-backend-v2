...
import { buildExamDisplayNames, buildScopeKeyFromExamType } from '../../shared/utils/examNaming';
...
import { institutionExamConfigService, type InstitutionExamRuntimeConfig } from '../../shared/institutions/exam-config';
...
export class ExamsService {
    ...
    private static readonly EXAM_START_RATE_LIMIT_WINDOW_SECONDS = Number.parseInt(
    ...
    private examHistoryCacheKey(userId: number, query: ExamHistoryQuery, version: string): string {
    ...
    private async enforceStartRateLimit(userId: number): Promise<void> {
        ...
        try {
            ...
            if (count === 1) {
                await cache.expire(key, ExamsService.EXAM_START_RATE_LIMIT_WINDOW_SECONDS);
            }
    ...
    private async acquireSubmitLock(userId: number, examId: number): Promise<string | null> {
        const cache = getCacheAdapter();
        ...
        try {
            const acquired = await cache.acquireLock(key, owner, ExamsService.EXAM_SUBMIT_LOCK_TTL_SECONDS);
            if (!acquired) {
                throw new AppError(
    ...
    private async releaseSubmitLock(examId: number, owner: string | null): Promise<void> {
        if (!owner) return;
        const cache = getCacheAdapter();
        if (!cache.available) return;
    ...
    private async nextExamSessionNumber(tx: any, userId: number, scopeKey: string): Promise<number> {
    ...
    private async getNextSessionNumber(userId: number, scopeKey: string): Promise<number> {
        return await prisma.$transaction(async (tx: any) => {
            return await this.nextExamSessionNumber(tx, userId, scopeKey);
        });
    }

    private resolveSoloExamType(input: StartExamInput, config: InstitutionExamRuntimeConfig): string {
        return institutionExamConfigService.resolveSoloExamType(input.examType, input.subjects.length, config);
    }
    ...
    private getMaxRetakesForExam(
        ...
        if (!isPremium && examType === EXAM_TYPES.REAL_PAST_QUESTION && this.isFullSoloExam(subjects)) {
            return (config?.freeFullRealTotalAttempts ?? FREE_TIER_LIMITS.FREE_FULL_REAL_TOTAL_ATTEMPTS) - 1;
        }
    ...
    private async getConfiguredExamDurationSeconds(exam: {
        ...
        return institutionExamConfigService.calculateDurationSeconds(exam.subjectsIncluded.length, config);
    }
    ...
    async checkExamEligibility(
        ...
        if (!user.isPremium && (examType === EXAM_TYPES.PRACTICE || examType === EXAM_TYPES.MIXED)) {
            return {
                canTakeExam: false,
                reason: examType === EXAM_TYPES.MIXED
        ...
        if (examType === EXAM_TYPES.REAL_PAST_QUESTION) {
            if (user.isPremium) {
                ...
                const examsToday: Array<{ subjectsIncluded: string[] }> = await prisma.exam.findMany({
                    where: {
                        userId,
                ...
                let maxCredits = config?.premiumDailyRealExamLimit ?? PREMIUM_LIMITS.DAILY_REAL_SUBJECT_CREDITS;

                // Handle legacy configs that might assume exam count instead of credits
                if (config?.premiumDailyRealExamLimit && config.premiumDailyRealExamLimit < 10) {
                ...
                return { canTakeExam: true, creditsUsed, creditsRemaining, requestedCredits };
            }
            ...
            const alreadyTaken = (user.freeSubjectsTaken ?? []).map(
                (s: string) => normalizeSubjectLabel(s).toLowerCase()
            );
            const requestedNormalized = subjects.map(
                (s: string) => normalizeSubjectLabel(s).toLowerCase()
            );
            const duplicateSubjects = requestedNormalized.filter(
                (s: string) => alreadyTaken.includes(s)
            );
    ...
    async startExam(
        ...
        const eligibility = await this.checkExamEligibility(userId, resolvedExamType, input.subjects, config); //check eligibility
        ...
        const questionsPerSubject = (isFreeUser && resolvedExamType === EXAM_TYPES.REAL_PAST_QUESTION)
            ? config.freeQuestionsPerSubject
            : config.questionsPerSubject;
        const totalQuestions = institutionExamConfigService.calculateTotalQuestions(input.subjects.length, {
            ...config,
            questionsPerSubject
        });
        const durationSeconds = institutionExamConfigService.calculateDurationSeconds(input.subjects.length, config);
        const useFeaturedFree =
            resolvedExamType === EXAM_TYPES.REAL_PAST_QUESTION && isFreeUser;
        const maxRetakes = this.getMaxRetakesForExam(Boolean(user?.isPremium), resolvedExamType, input.subjects, config);
        ...
        const scopeKey = buildScopeKeyFromExamType(resolvedExamType, input.subjects);
        ...
        const exam = await prisma.$transaction(async (tx: any) => {
            const sessionNumber = await this.nextExamSessionNumber(tx, userId, scopeKey);
            ...
            if (isFreeUser && resolvedExamType === EXAM_TYPES.REAL_PAST_QUESTION) {
                const normalizedSubjects = input.subjects.map(
                    (s: string) => normalizeSubjectLabel(s)
                );
        ...
        const naming = buildExamDisplayNames(resolvedExamType, input.subjects, exam.sessionNumber);
    ...
    async startDailyChallenge(
        ...
        if (idempotencyKey) {
            const routeKey = buildRouteKey('POST', '/api/exams/daily-challenge/start');
            return idempotencyService.execute(
                {
                    userId,
                    routeKey,
        ...
        if (existingChallenge) {
            throw new AppError("You have already completed today's Global Daily Challenge. Come back tomorrow!", 403);
        }
        ...
        if (selectedQuestionIds.length === 0) {
            throw new AppError("Unable to load daily challenge questions. Please try again later.", 500);
        }

        const rawQuestions: Array<QuestionWithMeta> = await prisma.question.findMany({
        ...
        const sessionNumber = await this.getNextSessionNumber(userId, EXAM_TYPES.DAILY_CHALLENGE);
        ...
        const exam = await prisma.$transaction(async (tx: any) => {
            const newExam = await tx.exam.create({
                data: {
                    ...
                    institutionId: null,
                    examType: EXAM_TYPES.DAILY_CHALLENGE as any,
                    nameScopeKey: buildScopeKeyFromExamType(EXAM_TYPES.DAILY_CHALLENGE as any, input.subjects),
                    sessionNumber,
                    subjectsIncluded: input.subjects,
        ...
        const naming = buildExamDisplayNames(EXAM_TYPES.DAILY_CHALLENGE as any, input.subjects, exam.sessionNumber);
    ...
    async submitExam(
        ...
        if (idempotencyKey) {
            const routeKey = buildRouteKey('POST', '/api/exams/:examId/submit', { examId });
            return idempotencyService.execute(
                {
                    userId,
                    routeKey,
        ...
        const examQuestionIds = new Set(exam.examAnswers.map((ea: any) => ea.questionId));
        ...
        const result = await prisma.$transaction(async (tx: any) => {
            ...
            if (finalizeExam.count !== 1) {
                throw new AppError('Exam has already been submitted or is no longer active.', 409, EXAM_ERROR_CODES.EXAM_ALREADY_COMPLETED);
            }
        ...
        const questionsWithAnswers: QuestionWithAnswer[] = exam.examAnswers.map((ea: any) => {
    ...
    async getExamQuestions(
        ...
        const questionsForClient: QuestionForClient[] = exam.examAnswers.map((ea: any) => ({
            id: ea.question.id,
            questionText: ea.question.questionText,
            hasImage: ea.question.hasImage,
            imageUrl: ea.question.imageUrl,
    ...
    async getExamHistory(
        ...
        const institution = await institutionContextService.resolveForUser(userId, query.institutionCode);
        const cache = getCacheAdapter();
        const version = (await cache.get(this.examHistoryVersionKey(userId))) ?? '0';
        ...
        const completedOriginalIds = Array.from(new Set(
            exams
                .filter((exam: any) => exam.status === EXAM_STATUS.COMPLETED)
                .map((exam: any) => (exam.isRetake && exam.originalExamId ? exam.originalExamId : exam.id))
        ));
        ...
        await setJson(cacheKey, response, ExamsService.EXAM_HISTORY_CACHE_TTL_SECONDS);
        return response;
    }
    ...
    async retakeExam(
        ...
        if (idempotencyKey) {
            const routeKey = buildRouteKey('POST', '/api/exams/:examId/retake', { examId });
            return idempotencyService.execute(
                {
                    userId,
                    routeKey,
        ...
        if (!user?.isPremium) {
            ...
            if (!isEligibleFreeRetake) {
                throw new AppError('Free users can only retake the full UI real exam. Upgrade to premium to unlock other retakes.', 403);
            }
        }

        const config = await institutionExamConfigService.getActiveConfigForInstitutionId(
            originalExam.institutionId ?? (await institutionContextService.resolveByCode()).id
        );

        const effectiveMaxRetakes = user?.isPremium
            ? (originalExam.maxRetakes ?? EXAM_CONFIG.MAX_RETAKES)
            : this.getMaxRetakesForExam(false, originalExam.examType, originalExam.subjectsIncluded, config);

        const eligibility = await checkRetakeEligibility(userId, examId, effectiveMaxRetakes);
        if (!eligibility.canRetake) {
            if (!user?.isPremium && eligibility.errorCode === EXAM_ERROR_CODES.MAX_RETAKES_REACHED) {
                throw new AppError(
        ...
        const durationSeconds = institutionExamConfigService.calculateDurationSeconds(originalExam.subjectsIncluded.length, config);
        const startedAt = new Date();
        const scopeKey = buildScopeKeyFromExamType(originalExam.examType as any, originalExam.subjectsIncluded as any);
        const expiresAt = new Date(
    ...
    async reportViolation(
        ...
        prisma.auditLog.create({
            ...
            console.warn('[ExamViolation] Failed to write audit log:', (err as Error).message);
        });
