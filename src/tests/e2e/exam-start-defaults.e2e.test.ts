  ...
  it("defaults single-subject exams to MIXED with a 13/12 source split and 22-minute timer", async () => {
    ...
    try {
      ...
      expect(body.data.timeAllowedSeconds).toBe(
        EXAM_CONFIG.SINGLE_SUBJECT_DURATION_SECONDS,
      );
      expect(body.data.displayNameShort).toBe("UI Mix • BIO • S001");

      const questionIds = body.data.questions.map(
        (question: { id: number }) => question.id,
      );
      const selectedQuestions = (await prisma.question.findMany({
        where: { id: { in: questionIds } },
        select: { questionType: true },
      })) as Array<{ questionType: string }>;

      const realCount = selectedQuestions.filter(
        (question) => question.questionType === "real_past_question",
      ).length;
      const practiceCount = selectedQuestions.filter(
        (question) => question.questionType === "practice",
      ).length;
      ...
      expect(
        selectedQuestions.every(
          (question: { questionType: string }) =>
            question.questionType === "real_past_question",
        ),
      ).toBe(true);
    } finally {
      await cleanupFixture(fixture);
