...
describe("Questions schema validation", () => {
  ...
  it("rejects a child question missing required answer choices", () => {
    ...
    expect(
      result.error.issues.some((issue) => issue.path.includes("optionA")),
    ).toBe(true);
    expect(
      result.error.issues.some((issue) => issue.path.includes("correctAnswer")),
    ).toBe(false);
  });
  ...
  it("requires all option fields when any answer choice is provided", () => {
    ...
    expect(result.success).toBe(false);
    expect(
      result.error.issues.some((issue) => issue.path.includes("optionD")),
    ).toBe(true);
  });
  ...
  it("rejects update payload when options and correct answer are partially provided for a child question", () => {
    ...
    expect(result.success).toBe(false);
    expect(
      result.error.issues.some((issue) => issue.path.includes("optionB")),
    ).toBe(true);
  });
});
